...
def merge_with_index_data(df, index_data, extra_fill_0_list=[]):
    ...
    c_columns = [
        '沪深300成分股', '上证50成分股', '中证500成分股', '中证1000成分股', '申万一级行业名称', '申万二级行业名称', '申万三级行业名称']
...
def transfer_to_period_data(df, period_type='m', extra_agg_dict={}):
    ...
    # resample 不光能使用rule作为参数，还能使用freq这个参数作为时间间隔进行重组。https://blog.csdn.net/weixin_40426830/article/details/111512471
