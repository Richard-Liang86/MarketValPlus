...
stock_code_list = get_stock_code_list_in_one_dir(
    root_path + '/data/daily_data/stock_for_select_targets')  # 获取目录内的所有股票的文件的路径
stock_code_list = [code for code in stock_code_list if 'bj' not in code]  # 排除北证版块的股票
print('股票数量：', len(stock_code_list))

# 导入上证指数，保证指数数据和股票数据在同一天结束，不然会出现问题。指数的最新数据如何获得，可以通过之前的课程学习
...
def calculate_by_stock(code):
    ...
    # =读入股票数据
    if operation == '选股':
        path = root_path + path_for_selc % code
    else:
        # path = root_path + '/data/daily_data/stock/%s.csv' % code
        path = root_path + path_for_cal % code
        # print(path)
        # exit()
    ...
    # =计算量价选股因子，里边计算了多个因子，例如是否成份股、市值因子==
    df = cal_tech_factor(df, extra_agg_dict)

    # =导入财务数据，并计算相关衍生指标，计算财务数据的代码比较复杂，可以深挖，可以通过config文件，选取自己所需要的字段，以下代码能用与字段的加工
    finance_df = import_fin_data(code, finance_data_path)
    if not finance_df.empty:  # 如果数据不为空
        # 计算财务数据：选取需要的字段、计算指定字段的同比、环比、ttm等指标、字段名可以参考https://www.quantclass.cn/data/stock/stock-fin-data-xbx
    ...
    # =将日线数据转化为月线或者周线, 如果我的代码选择在每周三下午卖出，周四上午买入，效果会如何？如果在周五下午换股，代码又应该怎样写？
...
if __name__ == '__main__':
    ...
    # 合并为一个大的DataFrame
    all_stock_data = pd.concat(df_list, ignore_index=True)  # 将读取到的数据进行上下合并
